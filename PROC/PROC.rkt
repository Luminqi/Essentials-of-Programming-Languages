...
(define var-in-exp
  (lambda (exp env) ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;should remove env
    (cases expression exp
      (const-exp (num) '())
      (var-exp (var) (list var))
      (diff-exp (exp1 exp2)
        (list (var-in-exp exp1) (var-in-exp exp2)))
      (if-exp (exp1 exp2 exp3)
        (list (var-in-exp exp1) (var-in-exp exp2) (var-in-exp exp3)))
      (let-exp (lovar loexp body)
        (removelist lovar (unique (flatten (list (map var-in-exp loexp) (var-in-exp body))))))
      (let*-exp (lovar loexp body)
        (let ((loval (value* lovar loexp env)))
          (value-of body
            (extend-env lovar loval env))))
      (minus-exp (exp) (var-in-exp exp))
      (add-exp (loexp)
        (map var-in-exp loexp))
      (mul-exp (loexp)
        (map var-in-exp loexp))
      (intq-exp (num exp) (var-in-exp exp))
      (emptylist-exp () '())
      (cons-exp (exp1 exp2)
        (list (var-in-exp exp1) (var-in-exp exp2)))
      (car-exp (exp)
