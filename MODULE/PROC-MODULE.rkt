...
;;;;;;;;;;;;;lang;;;;;;;;;;;;;;;;;;;;;;
  ...
  (define the-grammar
    '((program
      ...
      (declaration
       ("transparant" identifier "=" type)
       transparant-type-decl)
    ...
    (lambda (ty tenv)
      (cases type ty
        ...
        (proc-type (loarg-type result-type)
          (proc-type
            (map (lambda (type) (expand-type type tenv)) loarg-type)
        ...
        (list-type (ty1)
          (list-type (expand-type ty1 tenv)))
        (iface-type (lovar lotype)
          (iface-type
            lovar
            (map (lambda (type) (expand-type type tenv)) lotype)))
        (named-type (name)
          (lookup-type-name-in-tenv tenv name))
        (qualified-type (m-name t-name)
          (lookup-qualified-type-in-tenv m-name t-name tenv)))))
  ;;lookup-type-name-in-tenv : Tenv × Sym -> ExpandedType
  (define lookup-type-name-in-tenv
    (lambda (tenv t-name)
      (cases type-environment tenv
        (empty-tenv ()
          (eopl:error 'lookup-type-name-in-tenv "cannot find type ~s in tenv ~s" t-name tenv))
        (extend-tenv-with-type (name type saved-tenv)
          (if (equal? name t-name)
            type
            (lookup-type-name-in-tenv saved-tenv t-name)))
        (extend-tenv (vars types saved-tenv)
          (lookup-type-name-in-tenv saved-tenv t-name))
        (extend-tenv-with-module (name iface saved-tenv)  ;;;;;;need to be think more carefully
          (lookup-type-name-in-tenv saved-tenv t-name)))))
  ;;lookup-qualified-type-in-tenv : Sym × Sym × Tenv -> ExpandedType
      ...
      (let ((iface (lookup-module-name-in-tenv tenv m-name)))
        (cases interface iface
          ...
          (proc-iface (param-name param-iface result-iface)
            (eopl:error 'lookup-qualified-type-in-tenv "try to look up qualified-type in proc-iface"))))))
  ;;lookup-type-name-in-decls : Sym × Decls -> ExpandedType
  (define lookup-type-name-in-decls
    (lambda(t-name decls)
      (if (null? decls)
        (eopl:error 'lookup-type-name-in-decls "cant't find type ~s in decls ~s" t-name decls)
        (cases declaration (car decls)
          (transparant-type-decl (name ty)
            (if (equal? t-name name)
              ty
              (lookup-type-name-in-decls t-name (cdr decls))))
          (else
            (lookup-type-name-in-decls t-name (cdr decls)))))));;;what about opaque-type? Does the tenv/iface only contains transparant-type-decl
  (define apply-tenv
          ...
          (if (equal? search-var m-name)
            (cases interface iface
              (simple-iface (decls)
               ...
               (proc-iface (param-name param-iface result-iface)
                 (eopl:error 'apply-tenv "try to find the type of var ~s in proc-iface" search-var)))
            (apply-tenv saved-env search-var)))
        (extend-tenv-with-type (name type saved-env)
          (apply-tenv saved-env saved-env)))))
  ;;init-tenv : () -> tenv
  (define init-tenv
    (lambda ()
      (extend-tenv
    ...
    (lambda (m-name var-name tenv)
      (let ((iface (lookup-module-name-in-tenv tenv m-name)))
        (cases interface iface
          ...
          (proc-iface (param-name param-iface result-iface)
            (eopl:error 'lookup-qualified-var-in-tenv "try to look up the type of qualified-var in proc-iface"))))))
  ...
  (define type-to-external-form
    (lambda (ty)
      (cases type ty
        ...
        (named-type (name)
          (list 'named-type name))
        (qualified-type (m-name t-name)
          (list 'qualified-type 'from m-name 'take t-name)))))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;type-of-program;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
            ...
            (if (m-name-not-include-in-tenv tenv m-name)
              (let ((actual-iface (interface-of m-body (add-module-defns-to-tenv m-defns tenv))))
                (if (<:-iface actual-iface expected-iface tenv)
                  (let ((new-tenv (extend-tenv-with-module m-name (expand-iface m-name expected-iface tenv) tenv)))
          ...
          (opaque-type-decl (t-name)
            (let ((expanded-type (qualified-type m-name t-name)))
              (let ((new-tenv (extend-tenv-with-type t-name expanded-type internal-tenv)))
                (cons
                  (transparant-type-decl t-name expanded-type)
                  (expand-decls m-name (cdr decls) new-tenv)))))
          (transparant-type-decl (t-name ty)
  ...
  (define interface-of
    (lambda (m-body tenv)
      (cases module-body m-body
        ...
        (proc-module-body (param-name param-iface new-m-body)
          (let ((body-iface (interface-of new-m-body
                              (extend-tenv-with-module param-name (expand-iface param-name param-iface tenv) tenv))))
            (proc-iface param-name param-iface body-iface)))
  ...
  ;;report-attempt-to-apply-simple-module : Sym -> Unspecified
  (define report-attempt-to-apply-simple-module
    (lambda (rand-id)
      (eopl:error 'report-attempt-to-apply-simple-module "attempt to apply simple-module ~s" rand-id)))
  ;;report-bad-module-application-error : Interface × Interface × ModuleBody -> Unspecified
  (define report-bad-module-application-error
    (lambda (param-iface rand-iface m-body)
      (eopl:error 'report-bad-module-application-error "rand-iface ~s is not acceptable to app-module-nody ~s" rand-iface m-body)))
  ;;rename-in-iface : Interface × Sym × Sym -> Interface
  (define rename-in-iface
    (lambda (iface oldsym newsym)
      (cases interface iface
        (simple-iface (decls)
          (simple-iface (rename-in-decls decls oldsym newsym)))
        (proc-iface (param-name param-iface result-iface)
          (proc-iface param-name
            (rename-in-iface param-iface)
            (rename-in-iface result-iface))))))
  ;;rename-in-decls : Listof(Decl) × Sym × Sym -> Listof(Decl)
  (define rename-in-decls
    (lambda (decls oldsym newsym)
      (if (null? decls)
        '()
        (cases declaration (car decls)
          (val-decl (var type)
            (cons (val-decl var (rename-in-type type oldsym newsym)) (rename-in-decls (cdr decls) oldsym newsym)))
          (opaque-type-decl (t-name)
            (cons (opaque-type-decl t-name) (rename-in-decls (cdr decls) oldsym newsym)))
          (transparant-type-decl (t-name type)
            (cons (transparant-type-decl t-name (rename-in-type type oldsym newsym)) (rename-in-decls (cdr decls) oldsym newsym)))))))
  ;;rename-in-type : Type × Sym × Sym -> Type
  (define rename-in-type
    (lambda (ty oldsym newsym)
      (cases type ty
        (int-type () (int-type))
        (bool-type () (bool-type))
        (proc-type (lovar-type result-type)
          (proc-type
            (map (lambda (ty) (rename-in-type ty oldsym newsym)) lovar-type)
            (rename-in-type result-type oldsym newsym)))
        (pair-type (ty1 ty2)
          (pair-type
            (rename-in-type ty1 oldsym newsym)
            (rename-in-type ty2 oldsym newsym)))
        (list-type (ty1)
          (list-type (rename-in-type ty1 oldsym newsym)))
        (iface-type (lovar lotype)
          (iface-type (subst newsym oldsym lovar)
            (map (lambda (ty) (rename-in-type ty oldsym newsym)) lotype)))
        (named-type (t-name)
          (if (equal? t-name oldsym)
            (named-type newsym)
            ty))
        (qualified-type (m-name t-name)
          (let ((result (subst newsym oldsym (list m-name t-name))))
            (qualified-type (car result) (cadr result)))))))
  ;;subst : Sym × Sym × List -> List
  (define subst
    (lambda (new old lst)
      (cond
        ((null? lst) '())
                ...
                (let ((result-iface1 (rename-in-iface result-iface1 param-name1 new-name))
                  ...
                  (and
                    ...
                    (<:-iface result-iface1 result-iface2
                      (extend-tenv-with-module new-name (expand-iface new-name param-iface1 tenv) tenv)))))))))))
            ...
            (let ((name1 (decl->name (car decls1)))
              ...
              (if (equal? name1 name2)
                (and
                  ...
                  (loop (append cont (cdr decls1)) (cdr decls2) (extend-tenv-with-decl (car decls1) tenv) '()))
                (loop (cdr decls1) decls2 (extend-tenv-with-decl (car decls1) tenv) (append cont (list (car decls1)))))))))))
  ...
  (define fresh-module-name
    (let ((sn 0))
      (lambda (name)
        (set! sn (+ sn 1))
        (string->symbol (string-append (symbol->string name) (number->string sn))))))
  ...
  (define <:-decl
    (lambda (decl1 decl2 tenv)
      (or
        ...
        (and
          (transparant-type-decl? decl1)
          (transparant-type-decl? decl2)
          (equiv-type?
            (decl->type decl1)
            (decl->type decl2) tenv))
        (and
  ...
  (define val-decl?
    (lambda (decl)
      (cases declaration decl
        (val-decl (var type) #t)
        (else #f))))
  ;;transparant-type-decl? : Decl -> Bool
  (define transparant-type-decl?
    (lambda (decl)
      (cases declaration decl
        (transparant-type-decl (t-name type) #t)
        (else #f))))
    ...
    (lambda (decl)
      (cases declaration decl
        ...
        (transparant-type-decl (t-name type)
          type)
        (else
          (eopl:error 'decl->type "opaque-type-decl ~s is not accepateble" decl)))))
  ;;type-of : Exp × Tenv -> ExpandedType
  (define type-of
    (lambda (exp tenv)
      (cases expression exp
        ...
        (let-exp (lovar loexp body)
          (let ((lotype (map (lambda (exp1) (type-of exp1 tenv)) loexp)))
            (let ((loexpand-type (map (lambda (type) (expand-type type tenv)) lotype)))
              (type-of body (extend-tenv lovar loexpand-type tenv)))))
        (proc-exp (lovar lovar-type body)
          (let ((result-type
                  (type-of body
                    (extend-tenv lovar (map (lambda (type) (expand-type type tenv)) lovar-type) tenv))))
            (proc-type lovar-type result-type)))
        ...
        (letrec-exp (lop-result-type lop-name lob-vars lob-var-types lop-body letrec-body)
          (let ((lop-result-type (map (lambda (type) (expand-type type tenv)) lop-result-type))
                (lob-var-types (map (lambda (var-types) (map (lambda (type) (expand-type type tenv)) var-types)) lob-var-types)))
        ...
        (unpair-exp (var1 var2 p-exp body)
          (let ((p-type (type-of p-exp tenv)))
            (cases type p-type
              (pair-type (ty1 ty2)
                (type-of body
                  (extend-tenv (list var1 var2) (map (lambda (type) (expand-type type tenv)) (list ty1 ty2)) tenv)))
              (else
    ...
    (lambda (m-name var-name env)
      (let ((m-val (lookup-module-name-in-env m-name env)))
        (cases typed-module m-val
          ...
          (proc-module (b-var m-body saved-env)
            (eopl:error 'lookup-qualified-var-in-env "try to look up the value of qualified-var in proc-module"))))))
                    ...
                    (cases expval val
                      (module-val (module)
                        (cases typed-module module
                          ...
                          (proc-module (b-var m-body saved-env)
                            (eopl:error 'value-of "try to find the value of the qualified-var-exp in proc-module"))))
  ...
  (define value-of-module-body
    (lambda (iface m-body env)
      (cases module-body m-body
        ...
        (proc-module-body (pm-name pm-type pm-body)
          (proc-module pm-name pm-body env))
        (app-module-body (rator rand)
          (let ((rator-val
            ...
            (cases typed-module rator-val
              (proc-module (pm-name pm-body saved-env)
                (value-of-module-body iface pm-body
                  (extend-env-with-module pm-name rand-val saved-env)))
              (else
                (report-bad-module-app rator-val))))))))
  ;;report-bad-module-app : TypedModule -> Unspecified
  (define report-bad-module-app
    (lambda (tm)
      (eopl:error 'report-bad-module-app "try to apply the simple-module ~s in app-module-body" tm)))
            ...
            (if (null? decls)
              ...
              (cases declaration (car decls)
                ...
                (else
                  (loop (cdr decls)))))))
        (proc-iface (param-name param-iface result-iface)
          (eopl:error 'var-declared-in-interface "bad interface ~s" iface)))));;;;;;;;;;;;think carefully
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;data structures;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ;;;;;;;;;;;;;;;;;;;;;;;procedure datatype;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;let s1 = ints1.succ
;;in let z1 = ints1.zero
;;in let to-ints1 = ints1-to-int.to-int
;;in let s2 = ints2.succ
;;in let z2 = ints2.zero
;;in let to-ints2 = ints2-to-int.to-int
;;in let two1 = (s1 (s1 z1))
;;in let two2 = (s2 (s2 z2))
;;in -((to-ints1 two1), (to-ints2 two2))"0)
