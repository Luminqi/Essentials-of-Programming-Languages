...
(define first-of-let-cont
  (lambda (lovar loexp body env cont)
    (lambda (val)
      (if (null? loexp)
        (value-of/k body (extend-env lovar (list val) env) cont)
        (value-of/k (car loexp) env
          (rest-of-let-cont lovar (cdr loexp) (list val) body env cont))))))
;;rest-of-let-cont : loVar × loExp × loExpval × Exp × Env × Cont -> Cont
(define rest-of-let-cont
  (lambda (lovar loexp loval body env cont)
    (lambda (val)
      (if (null? loexp)
        (value-of/k body (extend-env lovar (append loval (list val)) env) cont)
        (value-of/k (car loexp) env
          (rest-of-let-cont lovar (cdr loexp) (append loval (list val)) body env cont))))))
...
;;rest-of-rand-cont : Proc × loExp × loExpval × Env × Cont
  ...
  (lambda (proc1 loval cont)
    (cases proc proc1
      (procedure (lovar body saved-env)
        (value-of/k body (extend-env lovar loval saved-env) cont)))))
