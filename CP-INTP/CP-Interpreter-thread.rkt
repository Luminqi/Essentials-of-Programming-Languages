...
;;exerceise5.52
;;(run "let x = 0
;;in let mut = mutex() mut1 = mutex() mut2 = mutex() mut3 = mutex()
;;in let incr_x = proc (id ownmut)
;;proc (dummy)
;;begin
;;wait(mut);
;;set x = -(x,-1);
...
;;wait(mut2);
;;wait(mut3);
;;spawn((incr_x 100 mut1));
;;spawn((incr_x 200 mut2));
;;spawn((incr_x 300 mut3));
;;wait(mut1);
;;wait(mut2);
;;wait(mut3);
;;x
