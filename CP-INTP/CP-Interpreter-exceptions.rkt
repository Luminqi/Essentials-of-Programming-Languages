(module CP-Interpreter-exceptions
(lib "eopl.ss" "eopl")
...
(define apply-handler
  (lambda (val cont)
    (cases continuation cont
      ...
      (rest-of-begin-cont (loexp env saved-cont)
        (apply-handler val saved-cont)))))
;;continuation-size : num × Cont -> num
...
;;(run "let f = proc (a) let g = proc (b) if zero?(-(a,b)) then 1 else raise 99 in proc(n) try (g n) catch (e) -1 in ((f 1) 2) ")
